...
let arraySum = function(array) {
  ...
  var additor = array[0];
  if (Array.isArray(additor)) {
    additor = arraySum(additor);
  }

  return additor + arraySum(array.slice(1));
};
...
let range = function(x, y) {
  ...
  if (num !== y) {
    return result.concat(num, range(num, y));
  }
};
...
let reverse = function(string) {
  ...
  return reversedString + string[lastIndex] + reverse(string.slice(0, lastIndex));
};
...
let countValuesInObj = function(obj, value) {
  ...
  for (key in obj) {
    ...
    if (typeof currentValue === 'object') {
      counter += countValuesInObj(currentValue, value);
    }
  }
...
let replaceKeysInObj = function(obj, oldKey, newKey) {
  ...
  for (key in obj) {
    ...
    if (key === oldKey) {
      ...
      currentValue = obj[newKey];
      delete obj[oldKey];
    }

    if (typeof currentValue === 'object') {
      currentValue = replaceKeysInObj(currentValue, oldKey, newKey);
    }
  }
